...
class Retina(Dataset):
    ...
    def __init__(self,
        ...
        self.img_path = glob(str(base_dir / image_folder / '*'))
        print('img_path', len(self.img_path))
        self.label_path = glob(str(base_dir / label_folder / '*'))
        print('label_path', len(self.label_path))
        ...
        for img in self.img_path:
            # Path.name获取文件名（不含路径），无需split分隔符
            filename = Path(img).name
            # 后续处理同原始逻辑（截取ID）
            img_id = filename.split('.')[0][5:]
            img_ids.add(img_id)
        label_ids = set()
        for label in self.label_path:
            filename = Path(label).name
            label_id = filename.split('.')[0][5:]
        ...
        self.imgs = sorted([
            img for img in self.img_path
            if Path(img).name.split('.')[0][5:] in matching_ids
        ])
        self.labels = sorted([
            label for label in self.label_path
            if Path(label).name.split('.')[0][5:] in matching_ids
        ])
